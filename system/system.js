...
const DEFAULT_SETTINGS = { screenMode: 'phone', language: 'fr-FR' };
export class System {
	constructor() {
	...
	/**
	 * Tester si au moins un des paramètres a été modifié
	 * @param {Boolean} reset - Réinitialiser le flag
	 * @returns Get flag state and reset flag
	 */
	hasSettingsChange(reset = true) {
		const changed = this.has_settings_change;
		if (reset) this.has_settings_change = false;
		return changed;
	}
