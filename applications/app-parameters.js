export const parametersApp = {
	...
	init: function (sys, windowId) {
        ...
        /** @type {JQuery<HTMLElement>} */
		const $screenSize = $(`#${windowId}`).find("#screen-size");
        /** @type {JQuery<HTMLElement>} */
		const $languageSelect = $(`#${windowId}`).find("#language-select");
		...
		// expose API
		return { pause: () => { }, resume: () => { }, restart: () => { updateScreenSize(); } };
	}
};
