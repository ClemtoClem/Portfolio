export const weatherApp = {
	...
	init: function (sys, windowId) {
		...
		geolocBtn.addEventListener('click', handleGeolocation); // Nouvel écouteur
		...
		/**
		 * Lance la recherche basée sur le nom de la ville
		 */
		...
		/**
		 * Lance la recherche basée sur la géolocalisation du navigateur
		 */
		function handleGeolocation() {
			...
			setLoadingState("Demande de votre position...");

			navigator.geolocation.getCurrentPosition(
				async (position) => {
					const { latitude, longitude } = position.coords;
					setLoadingState("Récupération du nom de la ville...");
					
					try {
						// 1. Géocodage inversé (Coordonnées -> Ville)
						...
						await fetchWeather(latitude, longitude, displayName);
						 ...
						 // Si le géocodage inversé échoue, on affiche quand même la météo
						 await fetchWeather(latitude, longitude, "Votre position actuelle");
					}
				},
				(error) => {
					...
					switch (error.code) {
						case error.PERMISSION_DENIED:
							setErrorMessage("Vous avez refusé l'accès à la localisation.");
							break;
						case error.POSITION_UNAVAILABLE:
							setErrorMessage("Impossible de déterminer votre position.");
							break;
						case error.TIMEOUT:
							setErrorMessage("La demande de localisation a expiré.");
							break;
						default:
							setErrorMessage("Erreur inconnue de géolocalisation.");
							break;
					}
				}
			);
		...
		/**
		 * Fonction principale pour récupérer la météo à partir des coordonnées
		 */
		async function fetchWeather(latitude, longitude, displayName) {
			setLoadingState("Chargement de la météo...");
			...
			if (!weatherData.daily) {
				setErrorMessage("Impossible de récupérer les données météo.");
				return;
			}
		...
		/**
		 * Convertit les coordonnées en nom de ville (Géocodage inversé)
		 */
		async function reverseGeocode(lat, lon) {
			 ...
			 // API retourne un objet, pas un tableau 'results'
			 if (data.name) {
		...
		function displayCurrentWeather(name, current) {
			cityNameEl.textContent = `Météo pour ${name}`;
